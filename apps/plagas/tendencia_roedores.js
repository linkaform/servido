  ...
  fetch(url + 'infosync/scripts/run/', {
    ...
    if (res.success) {
      if (res.response.json.array_filters.dispositivo.length){
        dispositivos = res.response.json.array_filters.dispositivo;
      ...
      if (res.response.json.catalogClient.length){
        ...
        for (i = 0; i < listSort.length; i++) {
          value = res.response.json.catalogClient[i]['663024da485975e0d21e5b44'];
          if(!listClients.includes(value)){
            listClients.push(value);
...
function set_option_dispositivos(){
  ...
  for (i = 0; i <list_options.length; i++) {
    let element = list_options[i];
    let dispositivo = element['dispositivo'];
    $('#dispositivo').append('<option value="'+ dispositivo +'"> Dispositivo '+dispositivo+'</option>');
  }
}