...
function loadDemoData(){
  ...
  getDrawTable('firstElement', columsTable1, dataTable1, 200);
  document.getElementById("firstElement").style.removeProperty('display');

  getDrawTable('secondElement', columsTable2, dataTable2, 180);
...
function getDrawTable(id, columnsData, tableData, height = 500){
    var  table = new Tabulator("#" + id, {
        ...
        rowFormatter: function(row) {
            var headers = document.querySelectorAll(".tabulator-col");
            ...
            row.getElement().style.backgroundColor = '#fff'; // color de fondo
            row.getElement().style.border = 'none';
            row.getElement().style.borderBottom = '1px solid #F4F6F9';
            row.getElement().style.color = '#000'; // color del texto
...
function drawGraphicFirst(data, setOptions){
  ...
  // Asignamos los colores y el borde al dataset
  data['datasets'][0]['backgroundColor'] = backgroundColors.slice(0, data['labels'].length);
  data['datasets'][0]['borderColor'] = borderColors.slice(0, data['labels'].length);
