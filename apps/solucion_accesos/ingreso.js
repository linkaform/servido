...
function getValidation(allData) {
  ...
  return res
	...
	const formContainerC = document.getElementById('formVehiculoContainer');

    function ocultarVehiculo() {
        formContainerC.classList.add('d-none');
    }
...
function AlertSendDataUser() {  
	...
	let vehiculos = vehiculosAgregados.map(({ id, ...rest }) => rest);
	...
	if (vehiculos.length > 0) {
		...
		vehiculos.forEach((vehiculo, index) => {
			...
			vehiculosHTML += `
				<div class="equipo-item">
					<div class="equipo-header" onclick="window.toggleVehiculoDetalle(${index})">
						<div class="equipo-title">
							<i class="fas fa-car-side" style="color: var(--primary-red); margin-right:8px;"></i>
							<span>${vehiculo.tipo || 'Vehículo'} - ${vehiculo.placas || 'Sin placas'}</span>
						</div>
						<i class="fas fa-chevron-down equipo-toggle"
						   style="color: var(--primary-red);"
						   id="toggle-vehiculo-${index}"></i>
					</div>
	
					<div class="equipo-details" id="vehiculo-${index}" style="display:none;">
	
						<div class="equipo-info-row">
							<span class="equipo-label">Tipo:</span>
							<span class="equipo-value">${vehiculo.tipo || '-'}</span>
						</div>
	
						<div class="equipo-info-row">
							<span class="equipo-label">Estado:</span>
							<span class="equipo-value">${vehiculo.estado || '-'}</span>
						</div>
	
						<div class="equipo-info-row">
							<span class="equipo-label">Marca:</span>
							<span class="equipo-value">${vehiculo.marca || '-'}</span>
						</div>
	
						<div class="equipo-info-row">
							<span class="equipo-label">Modelo:</span>
							<span class="equipo-value">${vehiculo.modelo || '-'}</span>
						</div>
	
						<div class="equipo-info-row">
							<span class="equipo-label">Placas:</span>
							<span class="equipo-value">${vehiculo.placas || '-'}</span>
						</div>
	
						<div class="equipo-info-row">
							<span class="equipo-label">Color:</span>
							<span class="equipo-value">${vehiculo.color || '-'}</span>
						</div>
	...
	Swal.fire({
		...
		<div class="modal-container">
			...
			<hr class="section-divider">
			<div class="section-header">
				<i class="fas fa-car-side" style="color: var(--primary-red); font-size:.9em;"></i>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!selectEstado) {
        console.error("No existe selectEstadoVehiculo en el DOM");
        return;
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const selectTipo = document.getElementById("selectTipoVehiculo");
    const selectMarca = document.getElementById("selectMarcaVehiculo");
    const selectModelo = document.getElementById("selectModeloVehiculo");

    if (!selectTipo || !selectMarca || !selectModelo) {
        console.error("Algún select no existe en el DOM");
        return;
    }

    if (!catalogoVehiculos || !catalogoVehiculos.length) {
        console.error("catalogoVehiculos no está definido o está vacío");
        return;
    }
    ...
    const tiposUnicos = [...new Set(
        catalogoVehiculos.map(v => v["Tipo de vehículo"].trim())
    )];
    ...
    selectTipo.addEventListener("change", function () {
        ...
        const marcasUnicas = [...new Set(
            catalogoVehiculos
                .filter(v => v["Tipo de vehículo"].trim() === tipoSeleccionado)
                .map(v => v["Marca"].trim())
        )];
    ...
    selectMarca.addEventListener("change", function () {
        ...
        const tipoSeleccionado = selectTipo.value.trim();
        ...
        const modelosUnicos = [...new Set(
            catalogoVehiculos
                .filter(v =>
                    v["Tipo de vehículo"].trim() === tipoSeleccionado &&
                    v["Marca"].trim() === marcaSeleccionada
                )
                .map(v => v["Modelo"].trim())
        )];
...
function setAddVehiculo() {
    ...
    const marca = document.getElementById('selectMarcaVehiculo')?.value;
    const modelo = document.getElementById('selectModeloVehiculo')?.value;
    ...
    if (!window.vehiculosAgregados) {
        window.vehiculosAgregados = [];
    }
