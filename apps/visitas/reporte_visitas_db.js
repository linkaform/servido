...
function getFirstElement(dateTo, dateFrom, promotor){
  ...
  .then(res => {
    if (res.success) {
      ...
      if (res.response.firtsElement.tabledata) {
        console.log("Estas dentro")
        console.log(res.response.firtsElement.tabledata)
        
        document.getElementById("firstParameters").style.removeProperty('display');

        getDrawTable('firstElement', columsTable1, res.response.firtsElement.tabledata, 350);
        document.getElementById("firstElement").style.removeProperty('display');
      }
    ...
    .then(res => {
      if (res.success) {
        if (res.response.catalog.length){
          console.log(res.response.catalog)
          array_value = []
          for (i = 0; i < res.response.catalog.length; i++) {
            if (!array_value.includes(res.response.catalog[i]['gestor'])) {
              array_value.push(res.response.catalog[i]['gestor'])
            }
          }
          array_value.sort();
          $("#promotor").empty();
