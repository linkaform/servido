...
function loadDemoData(){
  ...
  getDrawTable('seventhElement', columsTable5, dataTable5);
...
function getFirstElement(dateTo, dateFrom, paises, localidades, tiendas, usuario, check, flagPrint = false){
  ...
  .then(res => {
    if (res.success) {
      ...
      $("#divContent").show();
      $('.title_tables').show();
      if (res.response.data.firstElement) {
        tableFirstData = res.response.data.firstElement;
        getDrawTable('firstElement', columsTable1, res.response.data.firstElement, flagPrint);
        document.getElementById("firstElement").style.removeProperty('display');
      }
      if (res.response.data.secondElement) {
        getDrawTable('secondElement', columsTable2, res.response.data.secondElement, flagPrint);
        document.getElementById("secondElement").style.removeProperty('display');
      }
      if (res.response.data.thirdElement) {
        getDrawTable('thirdElement', columsTable3, res.response.data.thirdElement, flagPrint);
        document.getElementById("thirdElement").style.removeProperty('display');
      }
      if (res.response.data.fourthElement) {
        drawFourthElement(res.response.data.fourthElement, dataConfigFourth);
        document.getElementById("fivethElement").style.removeProperty('display');
      }
      if (res.response.data.fivethElement) {
        drawFivethElement(res.response.data.fivethElement, dataConfigFiveth);
        document.getElementById("sixthElement").style.removeProperty('display');
      }
      if (res.response.data.sixthElement) {
        getDrawTable('fourthElement', res.response.data.sixthElement.colums_data, res.response.data.sixthElement.data, flagPrint);
        document.getElementById("fourthElement").style.removeProperty('display');
      }
      console.log(res.response.data)
      if (res.response.data.seventhElement) {
        getDrawTable('seventhElement', columsTable5, res.response.data.seventhElement, flagPrint);
        document.getElementById("seventhElement").style.removeProperty('display');
      }
    } else {
      hideLoading();
